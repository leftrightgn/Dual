...
proj = r"C:\Users\it252195\source\repos\Dual\HEIN_Engine\HEIN_Engine.vcxproj"
    ...
    for item in item_group.findall("msbuild:None", ns):
        if item.attrib.get("Include") == "packages.config":
            ...
            if len(found_packages) > 1:
                ...
                print("Removed a duplicate packages.config!")

if len(found_packages) > 1:
    tree.write(proj, encoding="utf-8", xml_declaration=True)
    print("Saved file.")
else:
    print(f"Found {len(found_packages)} packages.config")
